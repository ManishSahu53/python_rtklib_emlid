...
def booldialogbox(message):
    result = tkMessageBox.askquestion("Provide you input",message, icon='warning')
    if result == 'yes':
        return 1
    else:
        return 0
...
# Reading zip files
base_zip  = tkFileDialog.askopenfilename(initialdir = "/",title = "Select Base ubx zip file",filetypes = (("zip files","*.zip"),("all files","*.*")));
rover_zip = tkFileDialog.askopenfilenames(initialdir = "/",title = "Select Rover ubx files",filetypes = (("zip files","*.zip"),("all files","*.*")));
